...
class backbone(nn.Module):
    def __init__(self, input_channel=1, hidden=48):
        ...
        self.layers2_1.add_module('layer_2_1_2', self._make_layer3d(BottleNeck3d, self.inplanes, 1, stride=(2, 1), size=self.ks[1], conv_num=1))
        ...
        self.layers2_2.add_module('layer_2_2_2', self._make_layer3d(BottleNeck3d, self.inplanes, 1, stride=(2, 1), size=self.ks[2], conv_num=1))
        ...
        self.is_ischemia = MLP(hidden*2*2, hidden, 1, 3) # bce
        # SPECT
        self.ischemia_intensity = MLP(hidden*2*2, hidden, 1, 3)  # regress
        self.ischemia_area = MLP(hidden*2*2, hidden, 1, 3) # regress
        self.xin_jian = MLP(hidden*2*2, hidden, 1, 3) # bce
        self.qian_bi = MLP(hidden*2*2, hidden, 1, 3) # bce
        self.jiange_bi = MLP(hidden*2*2, hidden, 1, 3) # bce
        self.xia_bi = MLP(hidden*2*2, hidden, 1, 3) # bce
        self.ce_bi = MLP(hidden*2*2, hidden, 1, 3) # bce
        # CTA
        self.LAD = MLP(hidden*2*2, hidden, 1, 3) # bce
        self.LCX = MLP(hidden*2*2, hidden, 1, 3) # bce
        self.RCA = MLP(hidden*2*2, hidden, 1, 3) # bce
    ...
    def forward(self, qrs, t):
        ...
        out_qrs = torch.cat([x2_1, x2_2], dim=1) 
