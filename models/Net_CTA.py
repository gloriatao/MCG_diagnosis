...
class backbone(nn.Module):
    def __init__(self, input_channel=1, hidden=48):
        ...
        # CTA
        self.trans_LAD_cta_t = VIT(n_embd=hidden*3, n_layer=6, n_head=4)
        self.trans_LCX_cta_t = VIT(n_embd=hidden*3, n_layer=6, n_head=4)
        self.trans_RCA_cta_t = VIT(n_embd=hidden*3, n_layer=6, n_head=4)
        
        self.trans_LAD_cta_qrs = VIT(n_embd=hidden*3, n_layer=6, n_head=4)
        self.trans_LCX_cta_qrs = VIT(n_embd=hidden*3, n_layer=6, n_head=4)
        self.trans_RCA_cta_qrs = VIT(n_embd=hidden*3, n_layer=6, n_head=4)
    ...
    def forward(self, qrs, t, return_feat=False):        
        ...
        # LAD
        feat_qrs_lad = self.trans_LAD_cta_qrs(feat_qrs)
        feat_t_lad = self.trans_LAD_cta_t(feat_t)
        feat_lad = self.fusion(feat_qrs_lad, feat_t_lad)     
        LAD = self.LAD_cta(feat_lad)
        
        # LCX
        feat_qrs_lcx = self.trans_LCX_cta_qrs(feat_qrs)
        feat_t_lcx = self.trans_LCX_cta_t(feat_t)
        feat_lcx = self.fusion(feat_qrs_lcx, feat_t_lcx)     
        LCX = self.LCX_cta(feat_lcx)
        
        # RCA
        feat_qrs_rca = self.trans_RCA_cta_qrs(feat_qrs)
        feat_t_rca = self.trans_RCA_cta_t(feat_t)
        feat_rca = self.fusion(feat_qrs_rca, feat_t_rca)    
        RCA = self.RCA_cta(feat_rca)
        
      
        output_cta = {'LAD':LAD, 'LCX':LCX, 'RCA':RCA}
        ouput_dict = {'is_ischemia':None, 'output_spect':None, 'output_cta':output_cta}
        
        if return_feat == False:        
            return ouput_dict
        elif return_feat:
            feat_LAD= feat_lad.unsqueeze(1)
            feat_LCX = feat_lcx.unsqueeze(1)
            feat_RCA = feat_rca.unsqueeze(1) 
