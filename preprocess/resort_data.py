...
# #----------non_ischemia and inshemia------------   
id = df1['心磁图编号'] 
Non_Ischemia, Ischamia = dict(), dict()
for i in range(id.shape[0]):
    one_id = id.values[i] 
    one_data = MCG_data[one_id]
    if one_data['is_ischemia'] == 1:
        # print(i, 'IS')
        Ischamia[one_id] = one_data
    elif one_data['is_ischemia'] == 0:
        # print(i, 'control')
        Non_Ischemia[one_id] = one_data
...
for i in range(id.shape[0]):
    one_id = id.values[i] 
    one_data = MCG_data[one_id]
    if (one_data['is_ischemia'] == 1) and (one_data['SPECT'] == True):
        Ischamia_SPECT[one_id] = one_data
...
for i in range(id.shape[0]):
    one_id = id.values[i] 
    one_data = MCG_data[one_id]
    if (one_data['is_ischemia'] == 1) and (one_data['CTA'] == True):
        Ischamia_CTA[one_id] = one_data   
        
print('# control:', len(Non_Ischemia))   
print('# spect is:', len(Ischamia_SPECT))   
print('# cta_is:', len(Ischamia_CTA)) 

print()    

with open('/media/cygzz/data/rtao/projects/MCG-NC/data/data001/data_pickle/Non_Ischemia.pickle', 'wb') as f:
    pickle.dump(Non_Ischemia, f)
f.close()

with open('/media/cygzz/data/rtao/projects/MCG-NC/data/data001/data_pickle/Ischamia_SPECT.pickle', 'wb') as f:
    pickle.dump(Ischamia_SPECT, f)
f.close()

with open('/media/cygzz/data/rtao/projects/MCG-NC/data/data001/data_pickle/Ischamia_CTA.pickle', 'wb') as f:
    pickle.dump(Ischamia_CTA, f)
f.close()
