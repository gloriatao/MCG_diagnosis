...
class Loss(nn.Module):
    ...
    def forward_gcn(self, output, gt, tag):
        is_ischemia_lss = self.get_bce(output['is_ischemia'].squeeze(1), gt['is_ischemia']) 
        # SPECT
        ...
        if tag == 'ischemia':
            ...
            loss_dict = {'LAD':LAD_lss}
        elif tag == 'RCA':
            loss_dict = {'RCA':RCA_lss }
        elif tag == 'LCX':
            loss_dict = {'LCX':LCX_lss}
        elif tag == 'xin_jian':
            loss_dict = {'xin_jian':xin_jian_lss}
        elif tag == 'qian_bi':
            loss_dict = {'qian_bi':qian_bi_lss}
        elif tag == 'jiange_bi':
            loss_dict = {'jiange_bi':jiange_bi_lss}
        elif tag == 'xia_bi':
            loss_dict = {'xia_bi':xia_bi_lss}
        elif tag == 'ce_bi':
            loss_dict = {'ce_bi':ce_bi_lss}
        return loss_dict
    
    def forward_loc(self, output, gt, tag, type='spect'):
        # 
        if type == 'spect':
            spect_idx = torch.where(gt['SPECT_idx']==1)[0]
            output_spect = output['output_spect']      
            ...
            ce_bi_lss = self.get_bce(output_spect['ce_bi'].squeeze(1)[spect_idx], gt['ce_bi'][spect_idx])  
            if tag == 'joint':
                loss_dict_joint = {'xin_jian':xin_jian_lss, 'qian_bi':qian_bi_lss, 'jiange_bi':jiange_bi_lss,
            ...
            RCA_lss = self.get_bce(output_cta['RCA'].squeeze(1)[cta_idx], gt['RCA'][cta_idx])
            if tag == 'joint':
                loss_dict_joint = {'LAD':LAD_lss, 'RCA':RCA_lss, 'LCX':LCX_lss}  
                        
        if tag == 'joint':
            loss_dict = loss_dict_joint             
        elif tag == 'LAD':
            loss_dict = {'LAD':LAD_lss}
        elif tag == 'RCA':
            loss_dict = {'RCA':RCA_lss}
